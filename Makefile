...
CFLAGS  := -c -mmcu=$(MCU) -Wall -Wshadow -std=gnu11 -Og -ggdb -DF_CPU=$(CPU_SPEED) -I$(FREE_RTOS_SRC_DIR) -I$(DRIVERS_INC_DIR) -I$(USER_CODE_INC_DIR)
LDFLAGS := -mmcu=$(MCU) -L$(DRIVERS_LIB_DIR) -lm -l$(DRIVERS_LIB_NAME)

ifeq ($(ENABLE_DEBUG_PRINT),true)
